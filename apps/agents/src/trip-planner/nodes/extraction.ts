...
export async function extraction(
  ...
  const schema = z.object({
    passengerFullName: z.string().describe("The full name of the passenger"),
    passengerEmail: z
      .string()
...
- passengerFullName - The full name of the passenger.
- passengerEmail - The email of the passenger.
- origin - The location to plan the trip from. Can be a city, state, or country.
- destination - The location to plan the trip to. Can be a city, state, or country.
...
Do NOT guess, or make up any information. If the user did NOT specify a destination location, their name or their email, please respond with a request for them to specify the this information.
You should ONLY send a clarification message if the user did not provide the destination location, their name or their email. 

You do NOT need any of the other fields, so if they're missing, proceed without them.
It should be a single sentence, along the lines of "Please specify the destination for the trip you want to go on".

If the user did not specify the origin location, default it to "New York".
If the user did not specify the number of guests, default it to 1.
